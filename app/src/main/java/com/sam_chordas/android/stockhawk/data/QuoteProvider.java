 ...
 * Edited by hussam.elemmawi on 13/09/16
 */
...
public class QuoteProvider {
    ...
    interface Path {
        ...
        String STOCK_VALUES = "stock_values";
    }
    ...
    @TableEndpoint(table = QuoteDatabase.STOCK_VALUES)
    public static class StockValues {
        @ContentUri(
                path = Path.STOCK_VALUES,
                type = "vnd.android.cursor.dir/stock_values"
        )
        public static final Uri CONTENT_URI = buildUri(Path.STOCK_VALUES);

        @InexactContentUri(
                name = "STOCK_VALUES_ID",
                path = Path.STOCK_VALUES + "/*",
                type = "vnd.android.cursor.item/stock_values",
                whereColumn = StockValuesColumns.SYMBOL,
                pathSegment = 1
        )
        public static Uri withSymbol(String symbol) {
            return buildUri(Path.STOCK_VALUES, symbol);
        }
    }
}
